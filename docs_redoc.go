...
// docs/redoc/LICENSE for ReDoc's license.
//
//go:embed docs/redoc/redoc.standalone.js
var redocAssets embed.FS

type redocUI struct{}

// ReDoc is a DocsUI backed by ReDoc, vendored under docs/redoc and
// embedded at build time. Pass it to MountDocs in place of SwaggerUI()
// for a single-page, three-panel doc layout instead of Swagger UI's
// try-it-out console.
func ReDoc() DocsUI { return redocUI{} }

func (redocUI) Assets() map[string]DocsAsset {
	return map[string]DocsAsset{
		"redoc.standalone.js": mustReadAsset(redocAssets, "docs/redoc/redoc.standalone.js", "application/javascript; charset=utf-8"),
	}
}
