...
func TestAdapter_CRUDAndDetailAction(t *testing.T) {
	engine := gin.New()
	r := &fakeResource{}
	r.Register(ginadapter.New(engine))
...
func TestAdapter_MountsUnderRouterGroup(t *testing.T) {
	engine := gin.New()
	group := engine.Group("/api/v1")
	r := &fakeResource{}
	r.Register(ginadapter.New(group))
...
// protectedResource mimics BaseResource.Protect declining a request —
// the handler itself never runs, proving auth/middleware wrapping works
// identically under gin since it only ever touches http.Handler.
...
func TestAdapter_ProtectedRouteReturns401(t *testing.T) {
	engine := gin.New()
	protectedResource{}.Register(ginadapter.New(engine))
